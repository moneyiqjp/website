...
abstract class MapSceneStoreCategory implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\MapSceneStoreCategoryTableMap';
    ...
    /**
     * Initializes internal state of Base\MapSceneStoreCategory object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>MapSceneStoreCategory</code> instance.  If
     * <code>obj</code> is an instance of <code>MapSceneStoreCategory</code>, delegates to
     * <code>equals(MapSceneStoreCategory)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|MapSceneStoreCategory The current object, for fluid interface
     */
    ...
    public function setSceneId($v)
    {
        ...
        if ($this->scene_id !== $v) {
            $this->scene_id = $v;
            $this->modifiedColumns[MapSceneStoreCategoryTableMap::COL_SCENE_ID] = true;
        }
    ...
    public function setStoreCategoryId($v)
    {
        ...
        if ($this->store_category_id !== $v) {
            ...
            $this->modifiedColumns[MapSceneStoreCategoryTableMap::COL_STORE_CATEGORY_ID] = true;
        }
    ...
    public function setPriorityId($v)
    {
        ...
        if ($this->priority_id !== $v) {
            $this->priority_id = $v;
            $this->modifiedColumns[MapSceneStoreCategoryTableMap::COL_PRIORITY_ID] = true;
        }
    ...
    public function setUpdateTime($v)
    {
        ...
        if ($this->update_time !== null || $dt !== null) {
            if ($dt !== $this->update_time) {
                $this->update_time = $dt;
                $this->modifiedColumns[MapSceneStoreCategoryTableMap::COL_UPDATE_TIME] = true;
            }
    ...
    public function setUpdateUser($v)
    {
        ...
        if ($this->update_user !== $v) {
            $this->update_user = $v;
            $this->modifiedColumns[MapSceneStoreCategoryTableMap::COL_UPDATE_USER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : MapSceneStoreCategoryTableMap::translateFieldName('SceneId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->scene_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MapSceneStoreCategoryTableMap::translateFieldName('StoreCategoryId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->store_category_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : MapSceneStoreCategoryTableMap::translateFieldName('PriorityId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->priority_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : MapSceneStoreCategoryTableMap::translateFieldName('UpdateTime', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->update_time = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : MapSceneStoreCategoryTableMap::translateFieldName('UpdateUser', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = MapSceneStoreCategoryTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\MapSceneStoreCategory'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildMapSceneStoreCategoryQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see MapSceneStoreCategory::setDeleted()
     * @see MapSceneStoreCategory::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildMapSceneStoreCategoryQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                MapSceneStoreCategoryTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_SCENE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'scene_id';
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_STORE_CATEGORY_ID)) {
            $modifiedColumns[':p' . $index++]  = 'store_category_id';
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_PRIORITY_ID)) {
            $modifiedColumns[':p' . $index++]  = 'priority_id';
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_UPDATE_TIME)) {
            $modifiedColumns[':p' . $index++]  = 'update_time';
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_UPDATE_USER)) {
            $modifiedColumns[':p' . $index++]  = 'update_user';
        }

        $sql = sprintf(
            'INSERT INTO map_scene_store_category (%s) VALUES (%s)',
    ...
    {
        $pos = MapSceneStoreCategoryTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['MapSceneStoreCategory'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['MapSceneStoreCategory'][$this->hashCode()] = true;
        $keys = MapSceneStoreCategoryTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getSceneId(),
            $keys[1] => $this->getStoreCategoryId(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\MapSceneStoreCategory
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setStoreCategoryId($arr[$keys[1]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_SCENE_ID)) {
            $criteria->add(MapSceneStoreCategoryTableMap::COL_SCENE_ID, $this->scene_id);
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_STORE_CATEGORY_ID)) {
            $criteria->add(MapSceneStoreCategoryTableMap::COL_STORE_CATEGORY_ID, $this->store_category_id);
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_PRIORITY_ID)) {
            $criteria->add(MapSceneStoreCategoryTableMap::COL_PRIORITY_ID, $this->priority_id);
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_UPDATE_TIME)) {
            $criteria->add(MapSceneStoreCategoryTableMap::COL_UPDATE_TIME, $this->update_time);
        }
        if ($this->isColumnModified(MapSceneStoreCategoryTableMap::COL_UPDATE_USER)) {
            $criteria->add(MapSceneStoreCategoryTableMap::COL_UPDATE_USER, $this->update_user);
        }
    ...
    {
        $criteria = ChildMapSceneStoreCategoryQuery::create();
        $criteria->add(MapSceneStoreCategoryTableMap::COL_SCENE_ID, $this->scene_id);
        $criteria->add(MapSceneStoreCategoryTableMap::COL_STORE_CATEGORY_ID, $this->store_category_id);
    ...
    public function hashCode()
    {
        $validPk = null !== $this->getSceneId() &&
            null !== $this->getStoreCategoryId();

        $validPrimaryKeyFKs = 2;
        $primaryKeyFKs = [];

        //relation FK_map_scene_store_scene to table scene
        ...
        //relation FK_map_scene_store_store to table store_category
        if ($this->aStoreCategory && $hash = spl_object_hash($this->aStoreCategory)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }
    ...
    public function setPrimaryKey($keys)
    {
        ...
        $this->setStoreCategoryId($keys[1]);
    }
    ...
    {
        return (null === $this->getSceneId()) && (null === $this->getStoreCategoryId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MapSceneStoreCategory (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \MapSceneStoreCategory Clone of current object.
     * @throws PropelException
     */
    ...
    public function setScene(ChildScene $v = null)
    {
        ...
        if ($v !== null) {
            $v->addMapSceneStoreCategory($this);
        }
    ...
    public function getScene(ConnectionInterface $con = null)
    {
        if ($this->aScene === null && ($this->scene_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aScene->addMapSceneStoreCategories($this);
             */
        }
    ...
    public function getStoreCategory(ConnectionInterface $con = null)
    {
        if ($this->aStoreCategory === null && ($this->store_category_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aStoreCategory->addMapSceneStoreCategories($this);
             */
        }
    ...
    {
        if (null !== $this->aScene) {
            $this->aScene->removeMapSceneStoreCategory($this);
        }
        if (null !== $this->aStoreCategory) {
            $this->aStoreCategory->removeMapSceneStoreCategory($this);
        }
        $this->scene_id = null;
    ...
    {
        return (string) $this->exportTo(MapSceneStoreCategoryTableMap::DEFAULT_STRING_FORMAT);
    }
