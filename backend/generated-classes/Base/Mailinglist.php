...
/**
 * Base class that represents a row from the 'mailinglist' table.
 *
 *
 *
...
abstract class Mailinglist implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\MailinglistTableMap';
    ...
    /**
     * Initializes internal state of Base\Mailinglist object.
     */
     ...
     *
     * @return $this|Mailinglist The current object, for fluid interface
     */
     ...
     * @return $this|\Mailinglist The current object (for fluent API support)
     */
    ...
    public function setUpdateTime($v)
    {
        ...
        if ($this->update_time !== null || $dt !== null) {
            if ($dt !== $this->update_time) {
                $this->update_time = $dt;
                $this->modifiedColumns[MailinglistTableMap::COL_UPDATE_TIME] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : MailinglistTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            $this->email = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MailinglistTableMap::translateFieldName('UpdateTime', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = MailinglistTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Mailinglist'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MailinglistTableMap::COL_UPDATE_TIME)) {
            $modifiedColumns[':p' . $index++]  = 'update_time';
        }

        $sql = sprintf(
            'INSERT INTO mailinglist (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MailinglistTableMap::COL_UPDATE_TIME)) {
            $criteria->add(MailinglistTableMap::COL_UPDATE_TIME, $this->update_time);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Mailinglist (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Mailinglist Clone of current object.
     * @throws PropelException
     */
